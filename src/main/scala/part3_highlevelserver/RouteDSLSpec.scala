...
class RouteDSLSpec
  ...
  // import object where Routing is specified
  ...
  "A digital library backend" should {
    ...
    "return a book by calling the endpoint with the id in the path" in {
      Get("/api/book/2") ~> libraryRoute ~> check {
        ...
        val strictEntity = Await.result(strictEntityFuture, 1 second) // if timeout passes , it throws an exception
    ...
    "not accept other methods than POST and GET" in {
      Delete("/api/book") ~> libraryRoute ~> check {
        ...
        // List(MethodRejection(HttpMethod(GET)), MethodRejection(HttpMethod(POST)))
      }
    }
